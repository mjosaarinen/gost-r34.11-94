...
uint8_t keygen_matrix[4][32][65];

/* mixing function matrix:  h' - h | m | s   (16-bit words) */
uint8_t mixing_matrix[16][48];

/* these are in gosthash.c */

extern uint32_t gost_sbox_1[256];
extern uint32_t gost_sbox_2[256];
extern uint32_t gost_sbox_3[256];
extern uint32_t gost_sbox_4[256];
...
{
  int i, j, z;
  uint32_t l, r, key[4][8], u[8], v[8], w[8];
...
void matrix_compress(uint32_t *h, uint32_t *m)
{
  int i, j, k;
  uint32_t key[8], s[8], v[65], t, r, l;
